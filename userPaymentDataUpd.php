...
$status = $_POST['status'];
$date = $_POST['date'];
$sql = "update `userpayment` set status = '$status', date = '$date' where userEmail = '$userEmail'";
$result = mysqli_query($con, $sql);
if($result){
...
<head>
    ...
    <title>Update User Payment</title>
</head>
<style>
body {
    margin: 0;
...
<body>
    ...
    <div class="lgcontainer my-5">
        <form class="container" method="post" onsubmit="return validateForm()" enctype="multipart/form-data">
            <center>
                <h2 style="color:#800000; padding: 5px;">User Payment update Form</h2>
            </center>
    ...
    function validateForm() {
        ...
        // Name validation
        if (name === '') {
            document.getElementById('name-error').textContent = 'Please select user payment status';
            isValid = false;
        }
        if (date === '') {
